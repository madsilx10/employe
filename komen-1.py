...
BEARER = 'AAAAAAAAAAAAAAAAAAAAANRILgAAAAAnNwIzUejRCOuH5E6I8xnZz4puTs%3D1Zv7ttfk8LF81IUq16cHjhLTvJu4FA33AGWWjCpTnA'
...
def follow_user(session):
    ...
    r = session.post(
        f'https://x.com/i/api/graphql/{FOLLOW_QUERY_ID}/FollowUser',
        json={'variables': {'userId': user_id}, 'queryId': FOLLOW_QUERY_ID}
    )
...
def post_comment(session, comment):
    ...
    r = session.post(
        ...
        json={'variables': variables, 'features': features, 'queryId': 'wUgPBh9hEKhMMGIg8uDuFw'}
    )
    if r.ok:
        data = r.json()
...
def process_account(account, comment, idx, done, followed):
...
def main():
    ...
    for i in range(start, end + 1):
        if i >= len(comments):
            ...
            continue
        process_account(accounts[i], comments[i], i+1, done, followed)
        if i < end:
            sleep(5, 35)
